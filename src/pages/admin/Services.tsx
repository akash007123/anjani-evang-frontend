                    ...
                    <tr key={id} className="hover:bg-slate-50/70 transition-colors">
                      ...
                      <td className="py-3.5 px-4">
                        <span className="flex items-center gap-1 font-mono text-slate-500">
                          <Hash className="w-3 h-3" />{svc.displayOrder}
                        </span>
                      </td>
                      <td className="py-3.5 px-4">
                        <span className="flex items-center gap-1 font-mono text-slate-600">
                          <DollarSign className="w-3 h-3" />{svc.pricePerGuest}/guest
                        </span>
                      </td>
                      <td className="py-3.5 px-4">
                        <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    min={0}
                    value={formData.pricePerGuest ?? 0}
                    onChange={(e) => setFormData({ ...formData, pricePerGuest: parseInt(e.target.value) || 0 })}
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 focus:outline-none focus:border-primary text-xs"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, minGuests: parseInt(e.target.value) || 25 })}
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 focus:outline-none focus:border-primary text-xs"
                  />
                </div>
        ...
        <div className="fixed inset-0 z-50 bg-secondary/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 max-h-[85vh] overflow-y-auto space-y-4 shadow-2xl border border-slate-200">
            ...
            <div className="flex flex-wrap gap-4 text-xs text-slate-600">
              <span className="flex items-center gap-1"><DollarSign className="w-3.5 h-3.5" />₹{viewingItem.pricePerGuest}/guest</span>
              <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5" />Min {viewingItem.minGuests} guests</span>
              <span className="flex items-center gap-1"><Hash className="w-3.5 h-3.5" />Order {viewingItem.displayOrder}</span>
            </div>
            <div
              className="text-xs text-slate-600 italic"
              dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(viewingItem.shortDescription || "") }}
            />
            {viewingItem.fullDescription && (
              <div
                className="prose prose-xs text-slate-700 border-t border-slate-100 pt-3"
                dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(viewingItem.fullDescription) }}
              />
            )}
