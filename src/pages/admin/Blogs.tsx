...
import { 
  ...
  ChevronLeft, ChevronRight, Check, AlertCircle, FileText, X, Star, Calendar, User, Tag
} from 'lucide-react';
...
export default function BlogsManagement() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!formData.title || !formData.content) {
      showToast('error', 'Please fill in required fields (Title and Full Content).');
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        showToast('success', id ? 'Blog post updated successfully' : 'Blog post published successfully');
        setIsFormOpen(false);
        fetchBlogs();
      } else {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-serif font-bold text-secondary">Blogs Management</h1>
          </div>
          <p className="text-slate-500 text-xs sm:text-sm mt-1">
            Create, edit, view, search, and curate dynamic blog articles for your website audience.
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-rose-500 space-y-2">
            ...
            <button onClick={fetchBlogs} className="text-xs text-primary underline mt-2 cursor-pointer">Retry</button>
          </div>
        ) : blogs.length === 0 ? (
          <div className="p-12 text-center text-slate-400 space-y-3">
            <FileText className="w-12 h-12 mx-auto text-slate-300" />
            <p className="text-base font-bold text-slate-600">No blog posts found</p>
            <p className="text-xs text-slate-400">Try adjusting your filters or add a new blog article.</p>
                    ...
                    <tr key={id} className="hover:bg-slate-50/70 transition-colors">
                      <td className="py-3.5 px-4 max-w-xs">
                        <div className="flex items-center gap-3">
                          <img
                            src={blog.featuredImage || 'https://images.unsplash.com/photo-1555244162-803834f70033?auto=format&fit=crop&q=80'}
                            alt={blog.title}
                            className="w-12 h-10 object-cover rounded-lg border border-slate-200 shrink-0"
                          />
                          <div className="truncate">
                            <p className="font-bold text-slate-900 truncate leading-snug">{blog.title}</p>
                      ...
                      <td className="py-3.5 px-4 text-slate-500 font-mono text-[11px]">
                        {new Date(blog.publishDate || blog.createdAt || Date.now()).toLocaleDateString()}
                      </td>
                      <td className="py-3.5 px-4">
                        <button
                          ...
                          title={blog.featured ? 'Featured on home' : 'Click to feature'}
                        >
                          <Star className={`w-3.5 h-3.5 ${blog.featured ? 'fill-amber-400' : ''}`} />
                        </button>
                      </td>
                      <td className="py-3.5 px-4">
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wide transition-all cursor-pointer ${
                            blog.status === 'Active' || blog.status === 'Published'
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl border border-slate-200">
            ...
            <form onSubmit={handleSave} className="space-y-4 text-xs font-sans">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  <label className="block font-bold text-slate-700 mb-1">Short Description / Excerpt *</label>
                  <textarea
                    required
                    rows={2}
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    placeholder="Write or paste full article HTML/Text content..."
      ...
      {viewingItem && (
        <div className="fixed inset-0 z-50 bg-secondary/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 max-h-[85vh] overflow-y-auto space-y-4 shadow-2xl border border-slate-200">
            ...
            />
            <p className="text-xs text-slate-600 italic">{viewingItem.excerpt}</p>
            <div className="prose prose-xs text-slate-700 border-t border-slate-100 pt-3" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(viewingItem.content || '') }} />
